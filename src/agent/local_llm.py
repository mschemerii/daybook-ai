...
DECOMPOSITION_PROMPT_VERSION = "task-decomposition-v7"
...
Every subtask title must be concise and contain no more than 50 characters. Descriptions and completion criteria must each contain no more than 4000 characters. estimated_hours must be a positive number in 0.25-hour increments and no greater than 1000.
Use short proposal-local item_key values, unique 1-based sequence numbers, quarter-hour estimates, and ISO dates or null.
prerequisite_item_keys may reference only other subtask item_key values in this proposal. Never include the current subtask's own item_key, and never create a dependency cycle.
If selected_task.due_date is null, every subtask due_date must be null. Otherwise each due_date must be null or an exact quoted ISO YYYY-MM-DD string such as "2026-08-18"; never use MM-DD-YYYY or other text.
The first subtask must use suggested_sequence 1, the second 2, and so on; never use 0. Every subtask priority must echo selected_task.user_priority exactly, regardless of the work described; the application owns and validates that inherited default.
